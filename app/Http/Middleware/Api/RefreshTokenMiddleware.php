 ...
 * @Description:
 * @Author: LMG
 * @Date: 2019-12-30 17:53:15
 * @LastEditors  : LMG
 * @LastEditTime : 2020-01-02 10:12:28
 */
...
class RefreshTokenMiddleware extends BaseMiddleware
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        //获取当前token
        $token = Auth::getToken();
        //即将过期了，也能获取到token里的载荷信息
        $payload = Auth::manager()->getJWTProvider()->decode($token->get());
        //如果不包含guard字段或者guard所对应的值与当前的guard的守护值不同，验证不通过
        if (empty($payload['guard']) || $payload['guard'] != $present_guard) {
            throw new TokenInvalidException();
        }
        //使用 try 包裹，以捕捉 token 过期所抛出的 TokenExpiredException  异常，此时进入的都是属于当前guard守护的token
        try {
            // 检测用户的登录状态，如果正常则通过
